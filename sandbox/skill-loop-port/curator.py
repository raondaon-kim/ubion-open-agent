...
# Original: https://github.com/NousResearch/hermes-agent/blob/b06e9993021a8eebd891fc60d52372446315b2f0/agent/curator.py
# Modifications:
#   - Initial vendor copy. No behavioral changes.
#   - Five lazy imports are resolved by sandbox shims or simply absent:
#       * `hermes_constants.get_hermes_home`  → agent_home.get_hermes_home (via hermes_constants.py shim)
#       * `from tools import skill_usage`     → top-level skill_usage.py (via tools/__init__.py shim)
#       * `from hermes_cli.config import load_config`           → missing, caught by existing try/except
#       * `from cron.jobs import rewrite_skill_refs`            → missing, caught by existing try/except
#       * `from agent import curator_backup`                    → missing, caught by existing try/except
#       * `from hermes_cli.runtime_provider import resolve_runtime_provider` → missing, caught by existing try/except
#       * `from run_agent import AIAgent`                       → run_agent.py shim re-exports mock_agent.AIAgent
#
