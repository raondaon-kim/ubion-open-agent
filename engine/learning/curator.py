...
# Modifications:
#   - Initial vendor copy on 2026-05-13 (sandbox/skill-loop-port).
#   - 2026-05-13: Promoted to engine/learning/curator.py.
#     Three direct imports rewritten to engine.* paths:
#       * `from hermes_constants import get_hermes_home`
#               → `from engine.storage.agent_home import get_hermes_home`
#       * `from tools import skill_usage`
#               → `from engine.skills import usage as skill_usage`
#       * `from run_agent import AIAgent`  (lazy, inside _run_llm_review)
#               → `from engine.core.agent import AIAgent`  (still lazy)
#     Other lazy imports kept as-is — they are wrapped in try/except in the
#     original code, so absent hermes_cli/cron/agent modules degrade
#     gracefully to no-op fallbacks. No behavioral changes.
#
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    from engine.skills import usage as _u
