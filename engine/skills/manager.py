...
# Original: https://github.com/NousResearch/hermes-agent/blob/b06e9993021a8eebd891fc60d52372446315b2f0/tools/skill_manager_tool.py
# Modifications: 7 import lines rewritten to engine.* equivalents.
#   Top-level (4):
#     - hermes_constants  -> engine.storage.agent_home  (get_hermes_home, display_hermes_home)
#     - utils             -> engine.storage.atomic      (atomic_replace, is_truthy_value)
#     - hermes_cli.config -> engine.skills._cfg_stub    (cfg_get; returns default when config absent)
#     - tools.registry    -> engine.tools.registry      (registry, tool_error)
#   Lazy inside function bodies (3):
#     - agent.skill_utils    -> engine.skills.utils       (L134, L299)
#     - agent.prompt_builder -> engine.core.prompt_builder (L780)
...
from engine.storage.agent_home import get_hermes_home, display_hermes_home
from typing import Dict, Any, Optional, Tuple

from engine.storage.atomic import atomic_replace, is_truthy_value
from engine.skills._cfg_stub import cfg_get
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from engine.skills.utils import get_all_skills_dirs
...
def skill_manage(
    ...
    if result.get("success"):
        try:
            from engine.core.prompt_builder import clear_skills_system_prompt_cache
