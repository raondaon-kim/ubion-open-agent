...
# Copyright (c) 2026 Ubion ax center (vendor copy, minimal modifications)
#
# Adapted from NousResearch/hermes-agent (MIT License)
# Original: https://github.com/NousResearch/hermes-agent/blob/b06e9993021a8eebd891fc60d52372446315b2f0/agent/skill_utils.py
# Modifications:
#   - Initial vendor copy on 2026-05-13 (Phase 1 Unit 2).
#   - Two import lines rewritten to engine.* paths:
#       * `from hermes_constants import get_config_path, get_skills_dir`
#               → `from engine.storage.agent_home import get_config_path, get_skills_dir`
#       * `from hermes_constants import get_hermes_home`  (lazy at line 239)
#               → `from engine.storage.agent_home import get_hermes_home`
#   - One lazy `from gateway.session_context import get_session_env` is
#     wrapped in try/except — we don't have the gateway module yet, and
#     the code path only triggers when skills.platform_disabled config is
#     set, which we don't use in Unit 2.
#
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    # Gateway module not vendored in Unit 2 — fall back to None when the
    # import fails. Only matters if skills.platform_disabled config is set,
    # which we don't use yet.
    try:
        from gateway.session_context import get_session_env  # type: ignore
    except ImportError:
        def get_session_env(_name):  # type: ignore[no-redef]
            return None
    resolved_platform = (
        platform
