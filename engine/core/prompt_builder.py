...
# Original: https://github.com/NousResearch/hermes-agent/blob/b06e9993021a8eebd891fc60d52372446315b2f0/agent/prompt_builder.py
# Modifications: top-level import paths rewritten to engine.* equivalents (3 lines).
#   - hermes_constants  -> engine.storage.agent_home  (get_hermes_home, get_skills_dir, is_wsl)
#   - agent.skill_utils -> engine.skills.utils        (7 symbols)
#   - utils             -> engine.storage.atomic      (atomic_json_write)
# Six lazy imports inside function bodies (tools.terminal_tool,
# tools.environments, gateway.session_context, hermes_cli.nous_subscription,
# tools.tool_backend_helpers, hermes_cli.config) are left unchanged — every
# call site is guarded by try/except so ImportError degrades gracefully on
# our Phase 1 surface (the corresponding modules are deferred to later units).
...
from engine.storage.agent_home import get_hermes_home, get_skills_dir, is_wsl
...
from engine.storage.atomic import atomic_json_write
