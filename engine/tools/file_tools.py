#!/usr/bin/env python3
# Vendor copy from NousResearch/hermes-agent (MIT). Six top-level import
# lines rewritten to point at our engine.* equivalents. Four lazy
# `from tools.terminal_tool import ...` calls inside function bodies are
# left untouched — three are already wrapped in try/except and degrade
# gracefully when terminal_tool is absent; the fourth lives in an env-
# manager code path that only fires when the caller explicitly creates
# an environment, so Phase 1 callers (poet-agent) never enter it.
...
from engine.tools.file_safety import get_read_block_error
from engine.tools.binary_extensions import has_binary_extension
from engine.tools.file_operations import (
    ShellFileOperations,
    normalize_read_pagination,
    normalize_search_pagination,
)
from engine.tools import file_state
from engine.learning.redact import redact_sensitive_text
