...
# Original: https://github.com/NousResearch/hermes-agent/blob/b06e9993021a8eebd891fc60d52372446315b2f0/tools/session_search_tool.py
# Modifications: 6 import lines rewritten to engine.* equivalents.
#   Top-level (2):
#     - agent.auxiliary_client -> engine.llm.aux_client  (async_call_llm, extract_content_or_reasoning)
#     - tools.registry         -> engine.tools.registry  (registry, tool_error)
#   Lazy inside function bodies (4):
#     - hermes_state          -> engine.storage.session_db  (SessionDB, format_session_db_unavailable, DEFAULT_DB_PATH)
#     - model_tools._run_async -> engine.llm.aux_client._run_async
# `hermes_cli.config.load_config` lazy import (L35) is left untouched —
# already guarded by try/except so ImportError falls back to defaults
# when our config layer is absent.
# Body otherwise byte-identical.
#
# Copyright (c) 2025 Nous Research (original algorithm + body)
# Copyright (c) 2026 Ubion ax center (import rewrites)
#
...
from engine.llm.aux_client import async_call_llm, extract_content_or_reasoning
...
def session_search(
    ...
    if db is None:
        try:
            ...
            from engine.storage.session_db import format_session_db_unavailable
    ...
    try:
        ...
        try:
            ...
            from engine.llm.aux_client import _run_async
...
def check_session_search_requirements() -> bool:
    ...
    try:
        from engine.storage.session_db import DEFAULT_DB_PATH
...
# --- Registry ---
from engine.tools.registry import registry, tool_error
